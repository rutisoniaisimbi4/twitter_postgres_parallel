...
def get_id_urls(url, connection):
    ...
    if res is None:
        sql = sqlalchemy.sql.text('select id_urls from urls where url=:url')
        res = connection.execute(sql, {'url': url}).first()
    return res[0]
...
def insert_tweet(connection, tweet):
    ...
    try:
        ...
        # POINT needs parentheses: POINT(lat lon)
        geo_coords = '(' + str(coords[0]) + ' ' + str(coords[1]) + ')'
        geo_str = 'POINT'
    except TypeError:
        try:
            # MULTIPOLYGON: each ring must be closed (last point == first point)
            geo_coords = '('
            for i, poly in enumerate(tweet['place']['bounding_box']['coordinates']):
                ...
                points.append(str(poly[0][0]) + ' ' + str(poly[0][1]))
                geo_coords += ','.join(points)
                geo_coords += '))'
            geo_coords += ')'
    ...
    # insert unhydrated user for in_reply_to_user_id if needed
    ...
    connection.execute(sql, {
        ...
        'geo':                   (geo_str + geo_coords) if geo_str is not None else None,
    })
